...
if(!$_POST){
    ...
    if (isset($_POST['insertar'])){
        ...
        if (isset($result)) {
            header('location:notas.php?grado='.$id_grado.'&materia='.$id_materia.'&seccion='.$id_seccion.'&revisar=1&info=1');
        } else {
            header('location:notas.php?grado='.$id_grado.'&materia='.$id_materia.'&seccion='.$id_seccion.'&revisar=1&err=1');
    ...
    //sino boton modificar que esta en el archivo alumnoedit.view.php
        ...
        //capturamos el id alumnos a modificar
            ...
            if (isset($result)) {
                header('location:alumnoedit.php?id=' . $id_alumno . '&info=1');
            } else {
                header('location:alumnoedit.php?id=' . $id_alumno . '&err=1');
            }// validación de registro
